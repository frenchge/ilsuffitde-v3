...
function buildGradient(pos: number, colors: string[], textColor: string) {
  ...
  const parts: string[] = [];
  const visibleBandStart = clampPct(bandStart);
  const visibleBandEnd = clampPct(bandEnd);

  if (bandStart > 0)
    parts.push(`${textColor} 0%`, `${textColor} ${visibleBandStart.toFixed(2)}%`);

  colors.forEach((c, i) => {
    const pct = n === 1 ? pos : bandStart + (i / (n - 1)) * BAND_HALF * 2;
    parts.push(`${c} ${clampPct(pct).toFixed(2)}%`);
  });

  if (bandEnd < 100)
    parts.push(`transparent ${visibleBandEnd.toFixed(2)}%`, "transparent 100%");
...
export function DiaTextReveal({
  ...
  return (
    <motion.span
      ...
      className={cn("relative inline-block align-bottom leading-[100%] text-inherit", className)}
      style={{
